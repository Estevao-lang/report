// ============================================================
// SnaveUK / MAIA Report Template
// Install once: npm install docx
// Run: node my-report.js
// ============================================================
const fs = require("fs");
const {
...
// ============ HELPERS ============
const txt = (t, o = {}) => new TextRun({ text: t, font: "Arial", size: 20, color: C.dark, ...o });
const code = (t) => new TextRun({ text: t, font: "Courier New", size: 20, color: C.accent });

const p = (t) => new Paragraph({ spacing: { after: 120 }, children: [txt(t)] });
const pMulti = (runs) => new Paragraph({ spacing: { after: 120 }, children: runs });
...
const bulletMulti = (runs) => new Paragraph({
  numbering: { reference: "bullets", level: 0 }, spacing: { after: 60 }, children: runs,
});
const numItem = (t) => new Paragraph({
  numbering: { reference: "numbers", level: 0 }, spacing: { after: 80 },
  children: [txt(t)],
});
const numItemMulti = (runs) => new Paragraph({
  numbering: { reference: "numbers", level: 0 }, spacing: { after: 80 }, children: runs,
});
const checkItem = (t) => new Paragraph({
  numbering: { reference: "checks", level: 0 }, spacing: { after: 50 },
  children: [txt(t)],
});
...
const divider = () => new Paragraph({
  ...
  border: { bottom: { style: BorderStyle.SINGLE, size: 2, color: C.medGray } }, children: [],
});
...
// Alert boxes (warning / danger / info / success)
...
const alertBoxMulti = (runs, type = "warning") => {
  const m = {
    warning: { fill: C.warningBg, b: C.warning },
    danger: { fill: C.dangerBg, b: C.danger },
    info: { fill: C.lightBg, b: C.accent },
...
const codeBlock = (lines) => {
  ...
  return new Table({
    ...
    rows: [new TableRow({
      children: [new TableCell({
        borders, shading: { fill: "F4F6F7", type: ShadingType.CLEAR },
        width: { size: W, type: WidthType.DXA },
        margins: { top: 100, bottom: 100, left: 200, right: 200 }, children: ch,
      })],
    })],
  });
};

// Quick table builder: rows as array of arrays, first row is header
...
// Build cover page (call inside section children array)
const cover = ({ kind, title, subtitle, meta }) => [
  spacer(600),
  ...
  spacer(200),
  pageBreak(), // CRITICAL: never forget this after the cover
];

// Build the document with standard styles & numbering
const buildDoc = ({ headerText, children }) => new Document({
  styles: {
...
// Save helper
const save = (doc, filename) => Packer.toBuffer(doc).then(buf => {
...
// Export everything
module.exports = {
  C, W, txt, code, p, pMulti, sec, sub, h3,
  bullet, bulletMulti, numItem, numItemMulti, checkItem,
  spacer, divider, pageBreak,
  hCell, dCell, buildTable, alertBox, alertBoxMulti, codeBlock,
  cover, buildDoc, save,
};
